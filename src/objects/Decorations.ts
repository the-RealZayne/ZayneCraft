...
outdoorsSubs.forEach(item => {
  ...
  drawProgressBar(leftX, y + barOffsetY, barWidth, barHeight, item.percent, item.color);
  y += rowGap;
});
...
skills.forEach(([name, percent]) => {
  ...
  drawProgressBar(rightX, y + barOffsetY, barWidth, barHeight, percent as number, '#3498db');
  y += rowGap;
});
